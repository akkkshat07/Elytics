...
class ValidationAgent:
  ...
  def process(self, state: QueryState) -> Dict[str, Any]:
    ...
    try:
      ...
      return {'sql_validation': validation, 'generated_sql': updated_sql, 'step_log': [f"{('' if is_valid else '')} Validator: valid={is_valid} | issues={len(all_issues)} | confidence={validation.get('confidence')}"]}
    except Exception as e:
      error_msg = f'ValidationAgent LLM review failed: {e}'
      logger.error(error_msg)
      is_valid = rule_pass
      return {'sql_validation': {'is_valid': is_valid, 'issues': rule_issues, 'corrected_sql': None, 'confidence': 'low', 'review_notes': f'LLM review failed, using rule-based result: {e}'}, 'step_log': [f' Validator: LLM review failed, rule-based={is_valid}']}