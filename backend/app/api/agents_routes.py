...
# Background job helpers (deduplicate cancellation logic)
# ---------------------------------------------------------------------------

async def _cancel_background_run(
    *,
    conv_service: ConversationService,
    client_id: str,
    run_id: str,
) -> bool:
    """Best-effort cancel of a background conversation."""
    cancellation_manager.signal_job(run_id)
    return await conv_service.cancel_background(run_id=run_id, client_id=client_id)


async def _cancel_active_background_runs_for_session(
    *,
    db,
    conv_service: ConversationService,
    client_id: str,
    ...
    """Cancel pending/running background conversations for a session."""
...
class StopStreamRequest(BaseModel):
    ...
    run_id: Optional[str] = Field(
        ...
        min_length=1,
        max_length=100,
        description="Run ID to cancel (preferred when available)",
    )
...
class AgentsRouter:
    ...
    def _add_agents_routes(self):
        ...
        async def agent_query_stream(
            ...
            dataset_id: Optional[str] = Query(None, description="Dataset to run the query against"),
            run_in_background: bool = Query(False, description="Force query to run as background job"),
            persona_slug: Optional[str] = Query(None, description="Agent persona slug to activate for this session"),
            ...
            try:
                ...
                if user_role != "super_admin":
                    ...
                    # Emit conversation_limit_warning notification exactly when usage CROSSES 90%
                    # current_count is before this query; after it completes it'll be current_count+1
                    if limit is not None and limit > 0:
                        after_this_query = current_count + 1
                        just_crossed_90 = (
                            after_this_query / limit >= 0.9 and
                            current_count / limit < 0.9
                        )
                        if just_crossed_90:
                            try:
                                ...
                                if _uid and client_id:
                                    ...
                                    await create_notification(_Notif(
                                        client_id=client_id,
                                        user_id=_uid,
                                        type="conversation_limit_warning",
                                        title="Conversation Quota Warning",
                                        message=f"You have used {after_this_query}/{limit} conversations this month ({remaining} remaining). Consider upgrading your plan.",
                                        metadata={"current_count": after_this_query, "limit": limit},
                                        target_role="any",
                                    ))
        ...
        async def get_available_personas(
            ...
            """
            Return the list of agent personas enabled for the current user's client,
            filtered to only those accessible under the client's active subscription tier.
            """
            client_id = current_user.get("client_id")
            if not client_id:
        ...
        async def submit_feedback(
            ...
            try:
                        ...
                        if feedback.rating.lower() == 'positive':
                            try:
                                ...
                                if success:
                                    ...
                                    # Vector DB indexing failed — log a warning but do NOT roll back.
                                    # The MongoDB feedback record is still valid; the query won't appear
                                    # in similarity search until vector DB recovers, but the user's
                                    # thumbs-up is recorded and the notification should still fire.
                                    self.logger.warning(
                                        f"Vector DB indexing failed for run_id {feedback.run_id} "
                                        f"(Qdrant may be unavailable). Feedback record will still be committed."
                                    )
                            except Exception as vec_error:
                                # Same as above: non-fatal — log and continue
                                self.logger.warning(
                                    f"Vector DB processing raised an exception for run_id {feedback.run_id}: {vec_error}. "
                                    f"Feedback record will still be committed."
                                )
                ...
                # Fire query_cached notification AFTER transaction commits successfully.
                # This is outside the transaction block so it always fires on positive
                # feedback regardless of whether Qdrant indexing succeeded.
                if feedback.rating.lower() == 'positive':
                    try:
                        from notifications.notification_service import create_notification
                        ...
                        if notif_user_id and client_id:
                            ...
                            await create_notification(Notification(
                                ...
                                type="query_cached",
                                title="Query Saved as Cache",
                                message=f'Your query "{short_q}" has been saved to the cache for faster future responses.',
                                metadata={"run_id": feedback.run_id},
                                target_role="any",
                            ))
                    except Exception as _ne:
        ...
        async def list_enabled_datasets(current_user: dict = Depends(require_auth())):
            """Return enabled datasets for the user's client (chat dropdown)."""
            from db_config.mongo_server import get_db
        ...
        async def get_suggested_questions(
            dataset_id: Optional[str] = Query(None, description="Load questions for this dataset's XML path"),
            current_user: dict = Depends(require_auth())
        ):
            """
            ...
            try:
                ...
                # Load client-specific suggested questions from data_sources directory
        ...
        async def suggest_questions(
            ...
            dataset_id: Optional[str] = Query(None, description="Dataset to scope suggestions to"),
            current_user: dict = Depends(require_auth()),
        ):
            """
            Typeahead suggestions from the client's cached questions.

            Performs keyword-based matching against the tenant's
            correct_responses.csv. Returns up to `limit` questions
            ranked by relevance score.

            GET /agents/suggest_questions?q=<partial query>&limit=5&dataset_id=<id>
            """
            client_id = current_user.get("client_id")
            if not client_id:
                raise HTTPException(
                    status_code=403,
                    detail="client_id missing from token",
                )
            ...
            try:
                ...
                self.logger.error(
                    f"suggest_questions error for client '{client_id}': {e}",
                    exc_info=True,
                )
                return {"suggestions": []}
        ...
        async def get_conversation_history(
            ...
            try:
                ...
                base_filter = {
                    ...
                    # Hide user-cancelled runs from UI history
                    "status": {"$ne": "cancelled"},
        ...
        async def agent_stop_stream(
            ...
            try:
                ...
                # Best-effort: mark the active conversation as cancelled so it doesn't render as completed.
                # This is intentionally NOT gated on `stopped` because the in-memory stream may already
                # be gone even though a pending/running conversation doc exists.
                try:
                    client_id = current_user.get("client_id")
                    if client_id:
                        await self.mongo_manager.connect()
                        ...
                        if db is not None:
                            ...
                            if run_id:
                                # Precise cancel: only cancel the run requested, and only if still active.
                                await collection.update_one(
                                    ...
                                    },
                                    {
                                        "$set": {
                                            ...
                                            "updated_at": now,
                                            "timing.end_time": now,
                                            "metadata.cancel_reason": "user_stop",
                                        }
                                    },
                                )
                            else:
                                # Fallback: atomically cancel the latest active run in the session.
                    ...
                    self.logger.warning(
                        f"Failed to persist cancelled status for session {session_id} (run_id={run_id}): {cancel_err}"
                    )
                ...
                try:
                    ...
                    if notif_uid and client_id:
                        ...
                        await create_notification(_Notif(
                            ...
                            type="query_pinned",
                            title="Query Pinned",
                            message=f'Your query "{convo_input}" has been pinned for quick access.',
        ...
        async def get_unread_session_count(
            current_user: dict = Depends(require_auth())
        ):
            """Count distinct sessions that have at least one unread conversation."""
            try:
                user_role = current_user.get("role", "user")
                client_id = current_user.get("client_id")
                is_admin = user_role in ["admin", "super_admin"]
                ...
                count = await self.mongo_manager.get_unread_session_count(
                    client_id=client_id,
                    user_id=user_id,
                )
        ...
        async def get_session_conversations(
            ...
            include_pending: bool = Query(True, description="Include pending/running conversations in the session thread"),
            current_user: dict = Depends(require_auth())
        ):
            """
            ...
            try:
                ...
                conversations = await self.mongo_manager.get_session_conversations(
        ...
        async def delete_session(
            ...
            try:
                ...
                # Delete should cance any live SSE stream for this session.
                try:
                    await self.orchestrator_manager.stop_stream(session_id)
                except Exception as stop_err:
                    self.logger.warning(
                        f"Failed to stop foreground stream for session {session_id}: {stop_err}"
                    )

                # Cancellation of any active background jobs for this session.
                try:
                    from db_config.mongo_server import get_db
                    db = await get_db()
                    conv_service = ConversationService(db)
                    await _cancel_active_background_runs_for_session(
                        db=db,
                        conv_service=conv_service,
                        client_id=client_id,
                        session_id=session_id,
                    ...
                    self.logger.warning(
                        f"Failed to cancel background jobs for session {session_id}: {bg_cancel_err}"
                    )
        ...
        async def internal_live_sql_execute(request: Request):
            """Execute live SQL using connector-managed sessions (SSH-aware)."""
            try:
                data = await request.json()
                ...
                creds = await DBCredentialsService(db).get_credentials(
                    client_id=client_id,
                    db_type=None,
                ...
                self.logger.info(
                    "live_sql_mode_selection client=%s dataset_id=%s mode=%s db_type=%s store_in_local=%s ssh_enabled=%s",
                    client_id, dataset_id, mode, db_type, store_in_local, ssh_enabled,
                )

                if store_in_local:
                    raise HTTPException(
                        status_code=400,
                        detail="Selected dataset is configured for local/parquet mode (store_in_local=true)",
                    )
                ...
                async def _llm_complete(prompt: str) -> str:
                    response = await llm.generate_completion(
                        system_prompt=(
                            "You are a SQL generator. Return ONLY one SELECT SQL query. "
                ...
                result = await execute_with_retries(
                    user_query=user_query if mode == "intent" else query,
                    session_factory=session_factory,
                    llm_complete=_llm_complete,
                    db_type=db_type,
                    schema_name=schema_name,
                    max_retries=max_retries if mode == "intent" else 0,
                    initial_query=query if mode == "query" else None,
                )

                df = result.get("dataframe")
                rows = df.to_dict(orient="records") if df is not None else []
                self.logger.info(
                    "live_sql_execution_result client=%s dataset_id=%s mode=%s status=%s row_count=%s query=%s",
                    client_id,
                    dataset_id,
                    mode,
                    result.get("status"),
        ...
        async def get_active_jobs(
            ...
            Reads from Redis first for low-latency responses; falls back to MongoDB when Redis
            is unavailable (e.g. first deploy, Redis outage).
            """
            try:
                client_id = current_user.get("client_id")
                user_id = current_user.get("user_id") or current_user.get("_id")
                ...
                if jobs is None:
                    ...
                    from db_config.mongo_server import get_db
                    db = await get_db()
                    jobs = await ConversationService(db).get_active_background(
        ...
        async def cancel_job(
            ...
            try:
                ...
                cancelled = await _cancel_background_run(
                    conv_service=conv_service,
                    client_id=client_id,
                    run_id=run_id,
                )
                ...
                from services import redis_job_store
                await redis_job_store.mark_job_terminal(
                    run_id=run_id,
                    client_id=client_id,
        ...
        # ══════════════════════════════════════════════════════════════════
        # DATAFRAME DOWNLOAD ENDPOINT
        # ══════════════════════════════════════════════════════════════════

        @self.router.get("/dataframe-download", tags=["Agents"])
        async def download_dataframe(
            run_id: str = Query(..., description="run_id of the conversation"),
            df_name: str = Query(..., description="DataFrame variable name, e.g. _generated_dataframe_1_"),
            current_user: dict = Depends(require_auth()),
        ):
            """
            Stream the full DataFrame (parquet → CSV) for large results that were
            truncated in the SSE payload.  The parquet file is resolved from the
            conversation record stored in MongoDB — the client never supplies a
            file path directly.
            """
            import csv
            import io as _io
            ...
            # Resolve the parquet path from MongoDB — never trust a client-supplied path
            mongo = self.mongo_manager
            doc = await mongo.db["conversations"].find_one(
                {"run_id": run_id, "client_id": client_id},
                {"agent_responses.executor": 1},
            )
            if not doc:
                raise HTTPException(status_code=404, detail="Conversation not found")
            ...
            if not parquet_path:
                raise HTTPException(
                    status_code=404,
                    detail="No full data file found for this DataFrame. It may not have been truncated.",
                )
            ...
            # Load parquet off the event loop so other requests/tabs aren't blocked
            # during potentially large file reads (sync pd.read_parquet blocks the loop).
            ...
            safe_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", df_name)
